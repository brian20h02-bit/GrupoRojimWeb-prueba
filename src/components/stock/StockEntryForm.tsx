...
export function StockEntryForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiFetch<{ movement: StockMovement }>("/api/stock/entries", {
        method: "POST",
        body: JSON.stringify({
          productId: selectedProduct.id,
          quantity: validQuantity,
          reason: supplier.trim() || undefined,
          reference: reference.trim() || undefined,
          notes: notes.trim() || undefined,
        }),
      });

      setSuccess(`Entrada registrada para ${selectedProduct.name}.`);
      setProducts((current) =>
        current.map((product) =>
      ...
      if (entryError instanceof ApiError) {
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      <section className="rounded-lg border border-luminoa-line bg-white p-5">
        ...
        <p className="mt-1 text-sm text-luminoa-muted">
          Selecciona un producto y registra la cantidad que ingresa al inventario.
        </p>

        <form className="mt-5 space-y-5" onSubmit={handleSubmit}>
          <div>
            <label htmlFor="entry-search" className="block text-sm font-medium text-slate-800">
              Producto
            </label>
            <input
              id="entry-search"
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <FormField label="Remito o factura opcional">
              <input
                value={reference}
        ...
        {selectedProduct ? (
          <div className="mt-5 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <StockValue label="Ingreso" value={validQuantity} />
              <StockValue label="Resultado" value={resultingStock} />
            </div>
            <StockBadge stock={resultingStock} stockMin={selectedProduct.stockMin} />
          </div>
        ) : (
